...
from kutilpy.util import KEnumType, Util
...
class ImmutableList(Generic[KListType]):
    ...
    def at(self, index: int) -> KListType:
        """指定したインデクスの要素を取り出します
    ...
    def at_or_else(self, index: int, default: Callable[[int], KListType]) -> KListType:
        """指定したインデクスの要素を取り出します
    ...
    def at_or_none(self, index: int):
        """指定したインデクスの要素を取り出します
        要素が存在しない場合Noneを返します
        :param index: 要素を取り出すインデックス
        :return: 取り出した要素
        """
        return self.at_or_else(index, lambda _: None)
    ...
    def join_to_str(self, separator: str, translator: Callable[[KListType, int], str] = lambda s: str(s)) -> str:
        ...
        result = translator(self.at(0), 0)
        for i in range(1, self.size()):
            result += separator + translator(self.at(i), i)
